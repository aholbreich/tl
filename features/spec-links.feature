...
Feature: Spec status for referenced specifications
  ...
  # A reference whose path ends in .feature is a spec reference. Spec state is
  # always resolved; there is no flag. The work is proportional to how much
  # Gherkin a project writes, so a project with none pays nothing and sees no
  # column — see .decisions/0002-reading-referenced-files.md.
  ...
  # -------------------------------------------------------------------------
  # Spec state, resolved on the ordinary read commands.
  # -------------------------------------------------------------------------
  ...
  Scenario: A task without a spec sits beside tasks that have one
    Given a task "task-abc123" with reference "features/login.feature"
    And a task "task-def456" with reference "src/auth/login.go"
    And the repository has a feature file "features/login.feature" with 3 scenarios
    When the developer runs `tl list`
    Then the output lists "task-def456" with no spec
  ...
  # -------------------------------------------------------------------------
  # A project that writes no Gherkin is unaffected — the column is not drawn.
  # -------------------------------------------------------------------------
  Scenario: A ledger with no spec references shows no spec column
    Given a task "task-abc123" with reference "src/auth/login.go"
    When the developer runs `tl list`
    Then the output does not contain "Spec"

  Scenario: The ready queue also reports spec state
    Given a task "task-abc123" with reference "features/login.feature"
    And the repository has a feature file "features/login.feature" with 3 scenarios
  ...
  # -------------------------------------------------------------------------
  # JSON — the key is always an array, never null, so consumers need no
  # presence check.
  # -------------------------------------------------------------------------
  Scenario: JSON exposes the resolved spec
    Given a task "task-abc123" with reference "features/login.feature"
    And the repository has a feature file "features/login.feature" with 3 scenarios
